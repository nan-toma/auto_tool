...
with open("config_tool_pass.txt") as f:
    ...
    # #TRIOSのpassword
    T_password = pass_file[3].replace("TRIOS_PASSWORD:", "").rstrip("\n") 

    ID = pass_file[0].replace("FMS_ID:", "").rstrip("\n") # ID = "toman01"

    #FMSのpassword
    password = pass_file[1].replace("FMS_PASSWORD:","").rstrip("\n") # password = "C9a553b98f"

    #TRIOSのID
    T_ID = pass_file[2].replace("TRIOS_ID:","").rstrip("\n")
...
messagebox.showinfo("pass",password)
